...
def set_x_bounds(n_shooting: int, abduction_range: dict, ode_solver) -> tuple[BoundsList, InitialGuessList]:
    ...
    q_init = np.concatenate(
        (
            np.linspace(abduction_range["min"], abduction_range["max"], int(n_shooting / 2) + 1),
            np.linspace(abduction_range["max"], abduction_range["min"], int(n_shooting / 2) + 1)[1:],
        ),
        axis=0,
    )
    ...
    qdot_init = np.concatenate(
        (np.linspace(0.0, 4.0, int(n_shooting / 4) + 1), np.linspace(4.0, -4.0, int(n_shooting / 2) + 1)[1:]), axis=0
    )
    ...
    humerus_q_max[: n_shooting // 2 - 100] = np.linspace(
        abduction_range["min"] + slack, abduction_range["max"] + slack / 4, n_shooting // 2 - 100
    )
    ...
    humerus_q_max[n_shooting // 2 - 100 : n_shooting // 2 + 100] = np.linspace(
        abduction_range["max"] + slack / 4, abduction_range["max"] + slack / 4, 200
    )

    humerus_q_max[n_shooting // 2 + 100 :] = np.linspace(
        abduction_range["max"] + slack / 4, abduction_range["min"] + slack, n_shooting // 2 - 99
    )
...
def set_u_bounds_and_init(n_shooting):
    ...
    u_bounds.add(
        key="muscles",
        min_bound=control_muscle_min,
        max_bound=control_muscle_max,
        phase=0,
        interpolation=InterpolationType.ALL_POINTS,
    )
    u_bounds.add(
    ...
    u_init.add(key="muscles", initial_guess=control_muscle_init, phase=0, interpolation=InterpolationType.ALL_POINTS)
    u_init.add(
