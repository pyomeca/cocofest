...
pulse_width_keys = list(data_minimize_fatigue["parameters"].keys())
muscle_names = ["BIClong", "BICshort", "TRIlong", "TRIlat", "TRImed", "BRA"]
nb_stim = len(data_minimize_fatigue["parameters"][pulse_width_keys[0]])
...
pw_list = []
for j in range(2):
    pw_list.append([pw_data_list[j][pulse_width_keys[i]] * 1000000 for i in range(len(pulse_width_keys))])
...
def plot_graph(datas):
    ...
    for i in range(len(pulse_width_keys)):
    ...
    fig.colorbar(scalarMap, ax=axs, orientation="vertical", label="pulse width (us)")
    plt.show()
