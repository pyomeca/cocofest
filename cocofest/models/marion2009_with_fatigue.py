...
class MarionModel2009WithFatigue(MarionModel2009):
    ...
    def __init__(
        ...
        ALPHA_A_DEFAULT = -4.0 * 10e-2  # Value from Ding's experimentation (s^-2)
        TAU_FAT_DEFAULT = 127  # Value from Ding's experimentation (s)
        ALPHA_TAU1_DEFAULT = 2.1 * 10e-6  # Value from Ding's experimentation (N^-1)
        ALPHA_KM_DEFAULT = 1.9 * 10e-6  # Value from Ding's experimentation (s^-1.N^-1)
    ...
    def declare_marion_with_fatigue_variables(
        self,
