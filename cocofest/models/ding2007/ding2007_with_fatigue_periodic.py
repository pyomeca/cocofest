...
class DingModelPulseWidthFrequencyWithFatiguePeriodic(DingModelPulseWidthFrequencyWithFatigue):
    """
    ...
    def __init__(
        ...
        self._stim_interval = stim_interval if stim_interval is not None else self._infer_stim_interval(stim_time)
    ...
    def _infer_stim_interval(stim_time: list[float] | None) -> float | None:
        ...
        if not np.allclose(deltas, reference):
            raise ValueError("The periodic Ding approximation requires evenly spaced stimulation times.")
        return reference
    ...
    def standard_rest_values(self) -> np.array:
        return np.array([[0], [0], [0], [self.a_scale], [self.tau1_rest], [self.km_rest]])
    ...
    def stimulation_decay_factor(self) -> float:
        if self._stim_interval is None:
            raise ValueError("stim_interval could not be inferred. Provide evenly spaced stim_time or pass stim_interval.")
